...
public static void editStudentDialog(String studentId, CStudentTable studentTable) {
    ...
    // Create ID number field (read-only)
    JTextField idNumberField = new JTextField(student.getIdNumber(), 20);
    ...
    dialog.add(new JLabel("Gender:"));
    dialog.add(genderDropdown);
    dialog.add(new JLabel("ID Number:"));
    dialog.add(idNumberField);
    dialog.add(new JLabel("Year Level:"));
    dialog.add(yearLevelDropdown);
    dialog.add(new JLabel("College:"));
    ...
    saveButton.addActionListener(e -> {
        ...
        // Validate inputs
        if (firstName.isEmpty() || lastName.isEmpty() || gender.isEmpty() || yearLevel.isEmpty()) {
            JOptionPane.showMessageDialog(dialog, 
                "All fields are required", 
                "Validation Error", 
                JOptionPane.ERROR_MESSAGE);
        ...
        // Save changes
        if (StudentDataManager.updateStudent(student)) {
            studentTable.refreshData();
            dialog.dispose();
            JOptionPane.showMessageDialog(
                dialog, 
